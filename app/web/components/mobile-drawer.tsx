...
export function MobileDrawer({ isOpen, onClose }: MobileDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 xl:hidden">
      ...
      {/* Drawer Panel */}
      <aside
        className="fixed inset-y-0 left-0 flex w-[285px] sm:w-[320px] max-w-full flex-col border-r border-slate-200 bg-white p-5 shadow-2xl transition-transform duration-200 ease-out"
        role="dialog"
        aria-modal="true"
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <h2 className="text-sm font-extrabold leading-tight text-slate-900 font-heading">
                SaaS Operativo
              </h2>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition"
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        onClick={onClose}
                        className={cn(
                          "flex items-center gap-3 rounded-2xl border px-3.5 py-2.5 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-cyan-500",
                          active
