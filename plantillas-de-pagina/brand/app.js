/* app.js - Interactividad para el sitio de Jairo Pinto */

// Configuración global de contacto
const CONFIG = {
  whatsappNumber: '573188430283', // Número de Jairo Pinto
  phoneNumber: '3188430283',     // Número de teléfono para llamadas directas
...
function initWhatsAppLinks() {
  ...
  actionButtons.forEach(btn => {
    ...
    if (actionType === 'product') {
      message = `Hola Jairo, quiero este producto: *${itemName}*.`;
    } else if (actionType === 'kit') {
      message = `Hola Jairo, quiero descuento en el *${itemName}*. ¿Cómo puedo adquirirlo?`;
    } else if (actionType === 'contact') {
      message = `Hola Jairo, quiero hablar contigo para conocer más sobre el Ganoderma Lucidum y Gano Excel.`;
    } else if (actionType === 'negocio') {
      message = `Hola Jairo, estoy en tu página de negocio y me gustaría recibir más información sobre la oportunidad de Gano Excel y cómo unirme al Nexus Team.`;
    }
...
/**
 * Inicialización del carrusel de videos nativos con ordenamiento aleatorio (Alternative 1)
 */
function initVideoCarousel() {
  ...
  const pixelsPerSecond = 110; // Velocidad de desplazamiento lineal exacta (110 píxeles por segundo)

  // 2. Loop de animación lineal basado en Delta Time (para suavidad perfecta e independiente de Hz)
  ...
  const startDrag = (clientX) => {
    ...
    startX = clientX;
    dragStartX = currentX;
    track.style.transition = 'none'; // Desactivar transiciones para respuesta física inmediata
  };
  ...
  const moveDrag = (clientX) => {
    ...
    if (Math.abs(diff) > 5) {
      hasDragged = true; // Activar bandera para ignorar reproducción accidental
    }
  ...
  const endDrag = () => {
    if (isDragging) {
      isDragging = false;
      lastTime = performance.now(); // Prevenir saltos temporales al soltar
    }
  };

  // Eventos de Mouse (PC)
  track.addEventListener('mousedown', (e) => {
    e.preventDefault(); // Evitar arrastre por defecto de imágenes/videos del navegador
    startDrag(e.clientX);
  });
  ...
  cards.forEach(card => {
    ...
    // Desactivar audio de todos los demás videos del carrusel y remover clases
    const muteAllOthers = () => {
      cards.forEach(c => {
    ...
    card.addEventListener('mouseleave', () => {
      if (video && !video.muted) {
        ...
        isPlaying = false;
        lastTime = performance.now(); // Prevenir saltos
        if (playBtn) {
    ...
    card.addEventListener('click', () => {
      if (hasDragged) return; // Ignorar si el usuario estaba arrastrando la cinta
      if (video && !video.muted) {
        video.muted = true;
        card.classList.remove('playing-audio');
        isPlaying = false;
