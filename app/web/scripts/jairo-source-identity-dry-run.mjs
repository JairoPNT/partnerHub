...
const CONFIRMATION = "DRY_RUN_JAIRO_SOURCE_IDENTITY";
const EXPECTED = {
...
export function buildSourceIdentityPlan({ source, canonicalBrand, manifestEntry, sourceHash, dependentHashes, productDestinationExists }) {
  ...
  return {
    ...
    projections: {
      product: { siteId: EXPECTED.productSiteId, ecosystemType: "PRODUCT", publicHost: "producto.jairopinto.pro" },
      brand: { siteId: EXPECTED.brandSiteId, ecosystemType: "PERSONAL_BRAND", publicHost: "brand.jairopinto.pro", source: "CANONICAL_TEMPLATE" }
    },
...
export async function runSourceIdentityDryRun({ sourceDirectory, canonicalBrandConfigPath, manifestPath, auditDirectory, now = new Date() }) {
  ...
  const backupDirectory = resolve(auditDirectory, `${now.toISOString().replaceAll(":", "-")}-jairo-source-identity-dry-run`);
  await mkdir(resolve(backupDirectory, "backup"), { recursive: true });
  await mkdir(resolve(backupDirectory, "projected"), { recursive: true });
  const writes = [
    writeFile(resolve(backupDirectory, "backup", "jairo-pinto.json"), sourceFile.source, "utf8"),
    ...
    writeFile(resolve(backupDirectory, "projected", "jairo-pinto-product.json"), `${JSON.stringify(plan.projectedProduct, null, 2)}\n`, "utf8"),
    writeFile(resolve(backupDirectory, "projected", "jairo-pinto.json"), `${JSON.stringify(plan.projectedBrand, null, 2)}\n`, "utf8")
  ];
  if (verification) writes.push(writeFile(resolve(backupDirectory, "backup", "jairo-pinto.verification.json"), verification.source, "utf8"));
  if (history) writes.push(writeFile(resolve(backupDirectory, "backup", "jairo-pinto.history.json"), history.source, "utf8"));
