...
import { buildBusinessWhatsappCta } from "@/server/services/businessWhatsappCta";
...
export const productPageGenerationInputSchema = z.object({
  ...
  cta: z.object({
    ...
    secondaryText: z.string().trim().min(1).optional(),
    guaranteeText: z.string().trim().min(1).optional(),
    directRegisterText: z.string().trim().min(1).optional()
  }).optional(),
  hero: z.object({
...
export function normalizeProductPageGenerationConfiguration(input: ProductPageGenerationInput) {
  ...
  const businessCta = input.ecosystemType === "BUSINESS"
    ? buildBusinessWhatsappCta({ whatsappNumber, defaultMessage }, input.cta)
    : undefined;
...
function buildConfigSource(configuration: ReturnType<typeof normalizeProductPageGenerationConfiguration>) {
...
async function applyConfiguredIntegrations(
  outputDirectory: string,
  configuration: ReturnType<typeof normalizeProductPageGenerationConfiguration>
) {
  const indexPath = resolve(outputDirectory, "index.html");
  const html = await readFile(indexPath, "utf8");
...
export const productPageGenerationService = {
  async generate(
    ...
    const baseConfiguration = normalizeProductPageGenerationConfiguration(input);
