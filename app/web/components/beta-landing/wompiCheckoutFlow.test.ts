...
test("buildWompiCheckoutUrl derives valid URL from server intent without client-side key generation", () => {
  ...
  const url = buildWompiCheckoutUrl(intent, "https://oferta.partnerhub.club", "/onboarding/tok-123");
  ...
  assert.ok(url.includes("redirect-url=https%3A%2F%2Foferta.partnerhub.club%2Fonboarding%2Ftok-123"));
});
